...
function deleteCookie(token, refreshToken) {
    document.cookie = `accessToken=${token};Path=/; Expires=Thu, 01 Jan 1970 00:00:01 GMT;`;
...
function getTokens() {
    ...
    if (cookies[0].startsWith("accessToken")) {
        token = cookies[0].split("accessToken=")[1];
        ...
        token = cookies[1].split("accessToken=")[1];
    }
